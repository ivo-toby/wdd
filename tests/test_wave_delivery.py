...
class WaveDeliveryStateTests(unittest.TestCase):
    ...
    def git_repository(self, root: Path) -> Path:
        ...
        self.git(repo, "config", "user.email", "wddctl@example.test")
        self.git(repo, "config", "user.name", "wddctl test")
    ...
    def test_status_and_render_are_deterministic(self):
        ...
        self.assertIn("Generated by wddctl", first)
    ...
    def test_review_run_validates_its_frozen_sha_output(self):
        ...
        command = [
            sys.executable,
            "-c",
            "import json; print(json.dumps({'schemaVersion': 1, 'kind': 'wddctl_review_result', 'task': 'TASK-001', 'baseSha': 'base-a', 'headSha': 'head-a', 'reviewer': 'test', 'findings': []}))",
        ]
        result = run_review(
            store,
            repo=root,
    ...
    def test_installer_creates_working_posix_and_windows_launchers(self):
        ...
        legacy_posix = bin_directory / "wdctl"
        legacy_windows = bin_directory / "wdctl.cmd"
        legacy_posix.write_text("legacy", encoding="utf-8")
        legacy_windows.write_text("legacy", encoding="utf-8")
        ...
        launcher = prefix / "bin" / "wddctl"
        self.assertTrue(launcher.exists())
        self.assertTrue((prefix / "bin" / "wddctl.cmd").exists())
        self.assertFalse(legacy_posix.exists())
        self.assertFalse(legacy_windows.exists())
