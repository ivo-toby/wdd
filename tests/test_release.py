"""Task 7 (2026-08-03 epic-scoped-state plan): CLI version identity and the
semver release automation (spec Sec3).

Pure-logic module + CAS loop, unit-tested with a fake `run_git` -- no
network, no real tags created against this repository's own git. Local
helpers only (no cross-file imports between test modules, per the
phase-6a/6b/7 test conventions).
"""

from __future__ import annotations

import contextlib
import importlib.util
import io
import sys
import unittest
from pathlib import Path
from typing import Any

ROOT = Path(__file__).resolve().parents[1]


def _load_release_module():
    """Load scripts/release.py by path (scripts/ has no package __init__)."""
    spec = importlib.util.spec_from_file_location("_wdd_release", ROOT / "scripts" / "release.py")
    module = importlib.util.module_from_spec(spec)
    assert spec.loader is not None
    spec.loader.exec_module(module)
    return module


def _load_installer_module():
    spec = importlib.util.spec_from_file_location(
        "_wdd_installer", ROOT / "scripts" / "install_wave_delivery.py"
    )
    module = importlib.util.module_from_spec(spec)
    assert spec.loader is not None
    spec.loader.exec_module(module)
    return module


def _load_version_module_at(version_py_path: Path):
    """Load `wave_delivery/version.py` from an arbitrary on-disk location.

    `wddctl_version()` resolves `VERSION` via `Path(__file__).resolve().
    parents[1]` -- loading the module by file location (like
    `_load_release_module` does for scripts/release.py) makes `__file__`
    the given path, so this genuinely exercises that resolution against
    whatever layout the caller built, not this repo's own real files.
    """
    spec = importlib.util.spec_from_file_location("_wdd_version_tmp", version_py_path)
    module = importlib.util.module_from_spec(spec)
    assert spec.loader is not None
    spec.loader.exec_module(module)
    return module


release = _load_release_module()


class _FakeGit:
    """Scripted `git` responses for CAS-loop tests.

    `local_tags` mirrors what a prior `git tag -f` (or an actual fetch of
    a pre-existing remote tag) has made visible locally -- used by
    `describe`. `remote_tags` is the authoritative server-side state,
    mutated only by a successful `push` -- used by `ls-remote`, matching
    `_tag_commit`'s deliberate choice to query the remote directly rather
    than trust local tag refs a rejected push may have left behind.

    `diff_has_changes` scripts `git diff --cached --quiet`: True (the
    default) means there IS a staged diff, so the real `git diff --cached
    --quiet` would exit 1 -> GitError; False means nothing is staged (the
    bootstrap-VERSION-already-matches case) -> exits 0, no exception.

    `moved_main` scripts the race `main.py`'s "main moved -> recompute and
    retry" arm exists for: on the FIRST `ls-remote` call, before returning,
    it mutates `origin_main`/`last_tag`/`remote_tags` to look like a
    concurrent, legitimate release landed between this attempt's fetch and
    this check -- exactly what a live `ls-remote` to origin (unlike a
    local, already-fetched ref) could actually observe.

    `log_messages_by_last_tag` lets a test vary what `git log` returns
    across attempts, keyed by the `last_tag` in effect when it's called --
    proving a retry genuinely recomputes rather than replaying the first
    attempt's answer with a new commit SHA.
    """

    def __init__(
        self,
        *,
        origin_main: str,
        last_tag: str | None,
        remote_tags: dict[str, str] | None = None,
        push_should_fail_times: int = 0,
        log_messages: list[str] | None = None,
        log_messages_by_last_tag: dict[str | None, list[str]] | None = None,
        diff_has_changes: bool = True,
        moved_main: dict[str, Any] | None = None,
        ls_remote_raises: bool = False,
    ) -> None:
        self.origin_main = origin_main
        self.local_head = origin_main
        self.last_tag = last_tag
        self.local_tags = dict(remote_tags or {})
        self.remote_tags = dict(remote_tags or {})
        self.push_should_fail_times = push_should_fail_times
        self.log_messages = log_messages if log_messages is not None else ["feat: add thing"]
        self.log_messages_by_last_tag = log_messages_by_last_tag
        self.diff_has_changes = diff_has_changes
        self.moved_main = moved_main
        self._moved_applied = False
        self.ls_remote_raises = ls_remote_raises
        self.push_calls = 0
        self.commit_counter = 0
        self.calls: list[list[str]] = []

    def __call__(self, args: list[str]) -> str:
        self.calls.append(list(args))
        cmd = args[0]
        if cmd == "fetch":
            return ""
        if cmd == "add":
            return ""
        if cmd == "diff":
            if self.diff_has_changes:
                raise release.GitError("diff --cached --quiet: changes present (exit 1)")
            return ""
        if cmd == "switch":
            self.local_head = self.origin_main
            return ""
        if cmd == "rev-parse":
            target = args[-1]
            if target == "origin/main":
                return self.origin_main
            if target == "HEAD":
                return self.local_head
            raise release.GitError(f"unhandled rev-parse target: {target}")
        if cmd == "describe":
            if "--exact-match" in args:
                for name, sha in self.local_tags.items():
                    if sha == self.origin_main:
                        return name
                raise release.GitError("no tag points at HEAD")
            if self.last_tag is None:
                raise release.GitError("no tags found")
            return self.last_tag
        if cmd == "log":
            if self.log_messages_by_last_tag is not None and self.last_tag in self.log_messages_by_last_tag:
                messages = self.log_messages_by_last_tag[self.last_tag]
            else:
                messages = self.log_messages
            return release._RECORD_SEP.join(messages + [""])
        if cmd == "commit":
            self.commit_counter += 1
            self.local_head = f"release-sha-{self.commit_counter}"
            return ""
        if cmd == "tag":
            # ["tag", "-f", name, sha]
            self.local_tags[args[2]] = args[3]
            return ""
        if cmd == "ls-remote":
            if self.ls_remote_raises:
                raise release.GitError("ssh: connect to host origin.example port 22: timed out")
            if self.moved_main is not None and not self._moved_applied:
                self._moved_applied = True
                self.origin_main = self.moved_main["origin_main"]
                self.last_tag = self.moved_main["last_tag"]
                self.remote_tags[self.moved_main["tag"]] = self.moved_main["tag_sha"]
            name = args[-1].rsplit("/", 1)[-1]
            sha = self.remote_tags.get(name)
            return f"{sha}\trefs/tags/{name}" if sha else ""
        if cmd == "push":
            self.push_calls += 1
            tag_name = args[-1]
            if self.push_should_fail_times > 0:
                self.push_should_fail_times -= 1
                # Simulate a competing push landing between our fetch and ours.
                self.origin_main = f"competing-sha-{self.push_calls}"
                raise release.GitError("! [rejected] main -> main (fetch first)")
            self.remote_tags[tag_name] = self.local_tags[tag_name]
            self.origin_main = self.local_head
            return ""
        raise release.GitError(f"unhandled git command: {args}")


class ParseCommitTest(unittest.TestCase):
    def test_feat_is_minor(self) -> None:
        self.assertEqual(release.parse_commit("feat: add widgets"), "minor")

    def test_fix_is_patch(self) -> None:
        self.assertEqual(release.parse_commit("fix: correct off-by-one"), "patch")

    def test_chore_is_patch(self) -> None:
        self.assertEqual(release.parse_commit("chore: tidy scripts"), "patch")

    def test_scoped_feat_is_minor(self) -> None:
        self.assertEqual(release.parse_commit("feat(api): add users endpoint"), "minor")

    def test_bang_on_any_type_is_major(self) -> None:
        self.assertEqual(release.parse_commit("feat!: drop legacy field"), "major")
        self.assertEqual(release.parse_commit("fix(core)!: change return type"), "major")

    def test_footer_only_breaking_change_is_major(self) -> None:
        message = (
            "fix: adjust retry timing\n\n"
            "Longer body describing the fix.\n\n"
            "BREAKING CHANGE: retry() no longer accepts a float timeout\n"
        )
        self.assertEqual(release.parse_commit(message), "major")

    def test_footer_hyphenated_breaking_change_is_major(self) -> None:
        message = "fix: x\n\nBREAKING-CHANGE: y\n"
        self.assertEqual(release.parse_commit(message), "major")

    def test_footer_breaking_change_escalates_even_with_nonconventional_header(self) -> None:
        # A malformed/non-conventional header used to short-circuit straight
        # to 'patch' before the footer was ever scanned -- the footer scan
        # must run regardless of whether the header itself parsed.
        message = "quick tweak, no conventional header\n\nBREAKING CHANGE: removed the old flag\n"
        self.assertEqual(release.parse_commit(message), "major")

    def test_breaking_change_footer_must_start_the_line(self) -> None:
        # `_BREAKING_FOOTER_RE` is anchored to line start (^ with
        # MULTILINE): quoted/embedded text mid-line must not escalate.
        message = 'fix: quote another commit\n\nSomeone wrote "BREAKING CHANGE: nope" inline.\n'
        self.assertEqual(release.parse_commit(message), "patch")

    def test_release_bump_trailer_overrides_header(self) -> None:
        message = "fix: minor cleanup\n\nRelease-Bump: major\n"
        self.assertEqual(release.parse_commit(message), "major")

    def test_release_bump_trailer_cannot_lower_a_breaking_change(self) -> None:
        # The trailer is a ratchet, not a downgrade path: it may only RAISE
        # the commit's own classification, never pull a `feat!:` (or a
        # BREAKING CHANGE footer) back down to something smaller.
        message = "feat!: big change\n\nRelease-Bump: minor\n"
        self.assertEqual(release.parse_commit(message), "major")

    def test_release_bump_trailer_still_raises_from_a_lower_base(self) -> None:
        message = "feat: add widgets\n\nRelease-Bump: major\n"
        self.assertEqual(release.parse_commit(message), "major")

    def test_squash_title_classified_by_its_own_header(self) -> None:
        message = "feat(api): add users endpoint (#42)\n\n* feat: add route\n* test: cover route\n"
        self.assertEqual(release.parse_commit(message), "minor")

    def test_bootstrap_no_tag(self) -> None:
        self.assertEqual(release.next_version(None, "major"), "0.1.0")
        self.assertEqual(release.next_version(None, "minor"), "0.1.0")
        self.assertEqual(release.next_version(None, "patch"), "0.1.0")

    def test_malformed_message_is_patch(self) -> None:
        self.assertEqual(release.parse_commit("fixed a thing"), "patch")
        self.assertEqual(release.parse_commit(""), "patch")
        self.assertEqual(release.parse_commit("   "), "patch")


class ComputeBumpTest(unittest.TestCase):
    def test_aggregates_to_highest_bump(self) -> None:
        messages = ["chore: tidy", "feat: add x", "fix: y"]
        self.assertEqual(release.compute_bump(messages), "minor")

    def test_merge_commit_subject_excluded_constituents_still_classified(self) -> None:
        messages = [
            "Merge pull request #7 from owner/feature-branch",
            "feat: add real feature",
        ]
        self.assertEqual(release.compute_bump(messages), "minor")

    def test_merge_branch_subject_excluded(self) -> None:
        messages = ["Merge branch 'develop' into main", "fix: small tweak"]
        self.assertEqual(release.compute_bump(messages), "patch")

    def test_empty_list_is_patch(self) -> None:
        self.assertEqual(release.compute_bump([]), "patch")

    def test_only_merge_commits_is_patch(self) -> None:
        self.assertEqual(release.compute_bump(["Merge pull request #1 from a/b"]), "patch")


class NextVersionTest(unittest.TestCase):
    def test_patch_bump(self) -> None:
        self.assertEqual(release.next_version("v1.2.3", "patch"), "1.2.4")

    def test_minor_bump_resets_patch(self) -> None:
        self.assertEqual(release.next_version("v1.2.3", "minor"), "1.3.0")

    def test_major_bump_resets_minor_and_patch(self) -> None:
        self.assertEqual(release.next_version("v1.2.3", "major"), "2.0.0")

    def test_no_prior_tag_bootstraps_regardless_of_bump(self) -> None:
        for bump in ("major", "minor", "patch"):
            self.assertEqual(release.next_version(None, bump), "0.1.0")

    def test_malformed_tag_raises(self) -> None:
        with self.assertRaises(ValueError):
            release.next_version("not-a-tag", "patch")


class DecideActionTest(unittest.TestCase):
    def test_no_existing_tag_is_push(self) -> None:
        self.assertEqual(release.decide_action(None, "sha-target"), "push")

    def test_tag_at_intended_commit_is_idempotent(self) -> None:
        self.assertEqual(release.decide_action("sha-target", "sha-target"), "idempotent")

    def test_tag_at_other_commit_is_foreign(self) -> None:
        self.assertEqual(release.decide_action("sha-other", "sha-target"), "foreign")


class TagCommitTransportErrorTest(unittest.TestCase):
    """I2: `_tag_commit` must fail closed on a real `ls-remote` transport
    failure, not swallow it and report "no tag" -- a network blip is not
    the same fact as a genuinely absent tag, and treating them alike would
    let the CAS loop push believing it had verified something it never
    checked."""

    def test_transport_error_propagates_instead_of_being_treated_as_absent(self) -> None:
        def raising_run_git(args: list[str]) -> str:
            raise release.GitError("ssh: connect to host origin.example port 22: timed out")

        with self.assertRaises(release.GitError):
            release._tag_commit(raising_run_git, "v1.0.0")

    def test_main_propagates_ls_remote_transport_errors(self) -> None:
        fake = _FakeGit(
            origin_main="sha-A0",
            last_tag="v1.0.0",
            remote_tags={},
            log_messages=["feat: add x"],
            ls_remote_raises=True,
        )
        with self.assertRaises(release.GitError):
            release.main(
                run_git=fake,
                write_version=lambda version: None,
                sleep=lambda seconds: None,
            )
        self.assertFalse(any(call[0] == "push" for call in fake.calls))


class ReleaseCASLoopTest(unittest.TestCase):
    """main()'s triage, driven end-to-end through a fake run_git."""

    def _run(self, fake: _FakeGit, **kwargs):
        stdout, stderr = io.StringIO(), io.StringIO()
        with contextlib.redirect_stdout(stdout), contextlib.redirect_stderr(stderr):
            code = release.main(
                run_git=fake,
                write_version=lambda version: None,
                sleep=lambda seconds: None,
                **kwargs,
            )
        return code, stdout.getvalue(), stderr.getvalue()

    def test_idempotent_at_head_short_circuits_before_any_commit(self) -> None:
        # The early "already tagged at HEAD" check: origin/main itself
        # already carries the intended tag, so this exits before the
        # decide_action triage is ever reached (covered separately below).
        fake = _FakeGit(
            origin_main="sha-A",
            last_tag="v1.2.0",
            remote_tags={"v1.2.0": "sha-A"},
        )
        code, out, _ = self._run(fake)
        self.assertEqual(code, 0)
        self.assertIn("idempotent", out)
        self.assertFalse(any(call[0] == "push" for call in fake.calls))
        self.assertFalse(any(call[0] == "commit" for call in fake.calls))

    def test_idempotent_via_decide_action_when_release_commit_matches_remote_tag(self) -> None:
        # Unlike the short-circuit above, origin/main is NOT at the
        # intended tag -- a release commit must be built first, and only
        # THEN does the remote tag turn out to already sit at that exact
        # commit (decide_action's own idempotent arm, reached via the
        # commit-carrying `release_commit`, not the pre-bump `head`).
        fake = _FakeGit(
            origin_main="sha-A",
            last_tag="v1.0.0",
            remote_tags={"v1.1.0": "release-sha-1"},
            log_messages=["feat: add x"],
        )
        code, out, _ = self._run(fake)
        self.assertEqual(code, 0)
        self.assertIn("idempotent", out)
        self.assertIn("v1.1.0", out)
        self.assertTrue(any(call[0] == "commit" for call in fake.calls))
        self.assertFalse(any(call[0] == "push" for call in fake.calls))

    def test_bootstrap_with_identical_version_skips_commit_but_still_tags(self) -> None:
        # C1: no prior tag, and VERSION on disk already reads "0.1.0" (a
        # fresh bootstrap) -- write_version produces byte-identical
        # content, so nothing is staged. A real `git commit` there would
        # exit 1 and raise; the CAS loop must instead skip straight to
        # tagging the current head with v0.1.0.
        fake = _FakeGit(
            origin_main="sha-boot",
            last_tag=None,
            remote_tags={},
            log_messages=["chore: init"],
            diff_has_changes=False,
        )
        code, out, _ = self._run(fake)
        self.assertEqual(code, 0, out)
        self.assertFalse(any(call[0] == "commit" for call in fake.calls))
        self.assertTrue(any(call[0] == "push" for call in fake.calls))
        self.assertEqual(fake.remote_tags["v0.1.0"], "sha-boot")

    def test_foreign_tag_fails_closed_and_names_both_commits(self) -> None:
        fake = _FakeGit(
            origin_main="sha-B",
            last_tag="v1.0.0",
            remote_tags={"v1.1.0": "sha-foreign"},
            log_messages=["feat: add x"],
        )
        code, _, err = self._run(fake)
        self.assertEqual(code, 1)
        self.assertIn("sha-foreign", err)
        # Named against the actual release commit the tag would have been
        # created at (release-sha-1), not the stale pre-bump head -- main
        # never moved in this scenario, so this is genuinely foreign.
        self.assertIn("release-sha-1", err)
        self.assertFalse(any(call[0] == "push" for call in fake.calls))

    def test_moved_main_triggers_recompute_and_retry_succeeds(self) -> None:
        # I2: the "main moved -> recompute and retry" arm. A concurrent,
        # legitimate release (v1.1.0) lands between this attempt's fetch
        # and its ls-remote check -- revealed live by ls-remote, not by
        # our already-fetched `origin/main`. The retry must be a REAL
        # second computation: last_tag has moved to v1.1.0, so attempt 2
        # sees different log messages and lands on a different version
        # (v1.1.1, not a re-hash of v1.1.0) -- proof it recomputed rather
        # than replaying attempt 1's answer with a new commit SHA.
        fake = _FakeGit(
            origin_main="sha-A0",
            last_tag="v1.0.0",
            remote_tags={},
            log_messages_by_last_tag={
                "v1.0.0": ["feat: add x"],
                "v1.1.0": ["fix: patch something small"],
            },
            moved_main={
                "origin_main": "sha-B0",
                "last_tag": "v1.1.0",
                "tag": "v1.1.0",
                "tag_sha": "competing-sha",
            },
        )
        code, out, _ = self._run(fake)
        self.assertEqual(code, 0, out)
        self.assertIn("pushed", out)
        # Attempt 1 never reached push -- it was recognized as a moved-main
        # race and recomputed, not as a push-time rejection.
        self.assertEqual(fake.push_calls, 1)
        self.assertEqual(fake.remote_tags["v1.1.0"], "competing-sha")
        # commit_counter is monotonic across the whole fake, so attempt 1's
        # (abandoned, never-pushed) commit still consumes "release-sha-1";
        # attempt 2's genuinely-pushed commit is "release-sha-2".
        self.assertEqual(fake.remote_tags["v1.1.1"], "release-sha-2")
        push_call = next(call for call in fake.calls if call[0] == "push")
        self.assertIn("--atomic", push_call)

    def test_exhausts_retries_and_fails_closed(self) -> None:
        fake = _FakeGit(
            origin_main="sha-A0",
            last_tag="v1.0.0",
            remote_tags={},
            push_should_fail_times=5,
            log_messages=["feat: add x"],
        )
        code, _, err = self._run(fake, max_attempts=3)
        self.assertEqual(code, 1)
        self.assertIn("exhausted", err)
        self.assertEqual(fake.push_calls, 3)


class VersionOutputShapeTest(unittest.TestCase):
    def test_wddctl_version_reads_version_file_one_directory_above_the_package(self) -> None:
        # Exercises the REAL `wddctl_version()` against a synthetic tmp
        # layout (repo-root/VERSION + repo-root/wave_delivery/version.py),
        # not this repo's own real files -- planting a VERSION content
        # that differs from this repo's actual VERSION proves the function
        # genuinely reads through `parents[1]` rather than merely agreeing
        # with a same-computed "expected" value by construction.
        import shutil
        import tempfile

        with tempfile.TemporaryDirectory() as tmp:
            layout_root = Path(tmp) / "lib"
            package_dir = layout_root / "wave_delivery"
            package_dir.mkdir(parents=True)
            shutil.copy(ROOT / "wave_delivery" / "version.py", package_dir / "version.py")
            (layout_root / "VERSION").write_text("9.9.9-test\n", encoding="utf-8")

            module = _load_version_module_at(package_dir / "version.py")
            self.assertEqual(module.wddctl_version(), "9.9.9-test")

    def test_wddctl_version_falls_back_when_version_file_is_unreadable(self) -> None:
        import shutil
        import tempfile

        with tempfile.TemporaryDirectory() as tmp:
            layout_root = Path(tmp) / "lib"
            package_dir = layout_root / "wave_delivery"
            package_dir.mkdir(parents=True)
            shutil.copy(ROOT / "wave_delivery" / "version.py", package_dir / "version.py")
            # No VERSION file written at all -- read_text() raises OSError.

            module = _load_version_module_at(package_dir / "version.py")
            self.assertEqual(module.wddctl_version(), "0.0.0+unknown")

    def test_cli_version_flag_prints_and_exits_zero(self) -> None:
        sys.path.insert(0, str(ROOT))
        try:
            from wave_delivery.cli import build_parser
        finally:
            sys.path.remove(str(ROOT))
        parser = build_parser()
        stdout = io.StringIO()
        with contextlib.redirect_stdout(stdout):
            with self.assertRaises(SystemExit) as raised:
                parser.parse_args(["--version"])
        self.assertEqual(raised.exception.code, 0)
        self.assertRegex(stdout.getvalue().strip(), r"^wddctl \d+\.\d+\.\d+(\+\S+)?( \(\S+\))?$")

    def test_doctor_report_includes_version_field(self) -> None:
        sys.path.insert(0, str(ROOT))
        try:
            from wave_delivery.doctor import inspect_capabilities
        finally:
            sys.path.remove(str(ROOT))
        payload = inspect_capabilities()
        self.assertIn("version", payload)
        self.assertRegex(payload["version"], r"^\d+\.\d+\.\d+(\+\S+)?$")


class InstallerCarriesVersionTest(unittest.TestCase):
    def test_install_copies_version_next_to_package(self) -> None:
        import tempfile

        installer = _load_installer_module()
        with tempfile.TemporaryDirectory() as tmp:
            prefix = Path(tmp) / "wdd"
            installer.install(prefix)
            installed_version = prefix / "lib" / "VERSION"
            self.assertTrue(installed_version.exists())
            self.assertEqual(
                installed_version.read_text(encoding="utf-8"),
                (ROOT / "VERSION").read_text(encoding="utf-8"),
            )


if __name__ == "__main__":
    unittest.main()
